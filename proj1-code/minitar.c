...
int create_archive(const char *archive_name, const file_list_t *files) {
    ...
    if(minitar == NULL){
        snprintf(err_msg, MAX_MSG_LEN, "Failed to open file for writing %s", archive_name);
        perror(err_msg);
        return -1;
    }
    const char *curr_file = files->head->name;
    tar_header *file_header = malloc(sizeof(tar_header));
    int tar_error = fill_tar_header(file_header, curr_file);
    if(tar_error != 0) {
        snprintf(err_msg, MAX_MSG_LEN, "Failed to create minitar header for %s", curr_file);
        perror(err_msg);
        return -1;
    }
    FILE *fp_curr = fopen(curr_file, "r");
    if(fp_curr == NULL) {
        snprintf(err_msg, MAX_MSG_LEN, "Failed to open file for reading: %s", curr_file);
        perror(err_msg);
        return -1;
    }
    size_t bytes_read;
    ...
    // once the bytes read is under 512 we know we hit the end, even if its a multiple of 512 the next block will be 0
    while((bytes_read = fread(stored_data, BLOCK_SIZE, 1, fp_curr)) < BLOCK_SIZE) {
        bytes_written = fwrite(stored_data,  BLOCK_SIZE, 1, minitar);
        if(bytes_written != bytes_read) {
            snprintf(err_msg, MAX_MSG_LEN, "Bytes lost while reading from %s and writing to %s", curr_file, archive_name);
            perror(err_msg);
            return -1;
        }
    }
